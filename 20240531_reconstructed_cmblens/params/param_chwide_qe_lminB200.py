...
lmin_tlm, lmin_elm, lmin_blm = (30, 30, 200) # The fiducial transfer functions are set to zero below these lmins
...
# ftl = cli(cls_len['tt'][:lmax_ivf + 1] + (nlev_t / 180 / 60 * np.pi) ** 2 * cli(transf_tlm ** 2)) * (np.arange(lmax_ivf + 1) >= lmin_tlm)
# fel = cli(cls_len['ee'][:lmax_ivf + 1] + (nlev_p / 180 / 60 * np.pi) ** 2 * cli(transf_elm ** 2)) * (np.arange(lmax_ivf + 1) >= lmin_elm)
# fbl = cli(cls_len['bb'][:lmax_ivf + 1] + (nlev_p / 180 / 60 * np.pi) ** 2 * cli(transf_blm ** 2)) * (np.arange(lmax_ivf + 1) >= lmin_blm)

# Same using unlensed spectra (used for unlensed response used to initiate the MAP curvature matrix)
# ftl_unl = cli(cls_unl['tt'][:lmax_ivf + 1] + (nlev_t / 180 / 60 * np.pi) ** 2 * cli(transf_tlm ** 2)) * (np.arange(lmax_ivf + 1) >= lmin_tlm)
# fel_unl = cli(cls_unl['ee'][:lmax_ivf + 1] + (nlev_p / 180 / 60 * np.pi) ** 2 * cli(transf_elm ** 2)) * (np.arange(lmax_ivf + 1) >= lmin_elm)
# fbl_unl = cli(cls_unl['bb'][:lmax_ivf + 1] + (nlev_p / 180 / 60 * np.pi) ** 2 * cli(transf_blm ** 2)) * (np.arange(lmax_ivf + 1) >= lmin_blm)
...
# Filtering out scales below lmin
